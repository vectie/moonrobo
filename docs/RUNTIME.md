...
The first shell is in `ui/cockpit`. It can render `sample-cockpit.json` directly
or import a fresh JSON snapshot generated by the native CLI. This keeps the
browser surface aligned with the same projection Rabbita will consume.
...
3. Port `ui/cockpit` into a Rabbita read-only cockpit package.
